...
export async function buyerApprovesRelease (wsProvider, timepoint=null) {
  const api = await ApiPromise.create({ provider: wsProvider });
  ...
  const signersKey = keyring.addFromSeed(
    Buffer.from(buyerHexSeed, 'hex'));

  const buyerAddress = await deriveAddress(buyerHexSeed);
  const sellerAddress = await deriveAddress(sellerHexSeed);
  const adminAddress = await deriveAddress(adminHexSeed);
  const otherSignatories = [sellerAddress, adminAddress].sort();
...
export async function sellerApprovesCancel (wsProvider, timepoint=null) {
  const api = await ApiPromise.create({ provider: wsProvider });
...
export async function adminFinalizeCancel (wsProvider, timePoint) {
  const api = await ApiPromise.create({ provider: wsProvider });
